...
export default function MainMap() {
  ...
  const userfriends = userStore.users.find(
    (user) => user._id == userStore.user._id
  ).friends;
  ...
  const makepins = (entries) => {
    ...
    for (let i = 0; i < entries.length; i++) {
      let found = unique.find(
        (entrie) =>
          entrie.lat == entries[i].location.lat &&
          entrie.lng == entries[i].location.lng
      );
      if (!found) {
        unique.push(entries[i].location);
      }
    }
    ...
    for (let i = 0; i < entries.length; i++) {
      for (let j = 0; j < locations.length; j++) {
        if (
          entries[i].location.lat == locations[j].location.lat &&
          entries[i].location.lng == locations[j].location.lng
        ) {
  ...
  const getFriendsEntries = (userfriends, entries) => {
    let friendsIds = userfriends.map((friend) => {
      return friend._id;
    });
    let friendsEntries = [];
  ...
  let myPins = makepins(myentries).map((marker, index) => (
    <Marker
      key={index}
      coordinate={{
      ...
      title={`${marker.entries.length} Memories`}
      description={"Click to view all Memories"}
      onCalloutPress={() => {
        navigation.navigate("PinEntries", { entries: marker.entries });
      }}
    />
  ));
  let friendsEntries = getFriendsEntries(userfriends, entries);
  let allEntries = myentries.concat(friendsEntries);
  let allPins = makepins(allEntries).map((marker, index) => (
    <Marker
      key={index}
      coordinate={{
        latitude: marker.location.lat,
  ...
  let buttons = [
    {
      title: "All",
      img: <FontAwesome5 name="users" size={24} color="black" />,
    },
    {
      title: "Me",
      img: (
        <Image
          ...
          source={{ uri: `${baseURL}${userStore.user.profileImage}` }}
          alt="Alternate Text"
          size="xs"
        />
      ),
  ...
  const filterMapPins = (title, img, _id) => {
    if (title == "All") {
      ...
      let friendEntries = entries.filter(
        (entry) => entry.user == _id && entry.isPriv == false
      );
      let friendPins = makepins(friendEntries).map((marker, index) => (
        <Marker
          key={index}
          coordinate={{
            latitude: marker.location.lat,
  ...
  return (
    ...
    >
      <View style={{ flex: 1, backgroundColor: "white" }}>
        ...
        <MapView provider={PROVIDER_GOOGLE} style={styles.map} region={view}>
          {/* pins to be rendered */}
          {viewPins}
          <Marker
            // key={index}
        ...
        >
          <MaterialIcons name="my-location" size={25} color="#1a73e8" />
        </Button>
        <Button
          onPress={() => {
            toggleModal();
            ...
            <FlatList
              ...
              renderItem={({ item: button }) => (
                <View style={{ flex: 0.25, flexDirection: "column" }}>
                  <Button
                    onPress={() => {
                      filterMapPins(button.title, button.img, button._id);
                    }}
                    bgColor={"#D9D9D9"}
                    style={{
                      alignSelf: "center",
